  ...
  useEffect(() => {
    const fetchPerfil = async () => {
      try {
        ...
        // Detectar tipo de cédula y extraer valor sin prefijo
        if (usuario.cedula?.startsWith('CR-')) {
          setTipoIdentificacion('nacional');
          usuario.cedula = usuario.cedula.replace('CR-', '');
        } else if (usuario.cedula?.startsWith('RES-')) {
          setTipoIdentificacion('residente');
          usuario.cedula = usuario.cedula.replace('RES-', '');
        } else if (usuario.cedula?.startsWith('PAS-')) {
          setTipoIdentificacion('extranjero');
          usuario.cedula = usuario.cedula.replace('PAS-', '');
        }

        // Detectar tipo de teléfono y extraer prefijo/número
        if (usuario.telefono?.startsWith('+506-')) {
          setTipoTelefono('nacional');
          usuario.telefono = usuario.telefono.replace('+506-', '');
          usuario.telefonoPrefijo = ''; // Se ignora prefijo para nacional
        } else if (usuario.telefono?.startsWith('+')) {
          setTipoTelefono('extranjero');
          const [, pref, num] = usuario.telefono.match(/^\+(\d{1,3})-(\d{7,12})$/) || [];
          usuario.telefono = num || '';
          usuario.telefonoPrefijo = pref || '';
        }
...
const telefonoFinal =
  tipoTelefono === 'nacional'
    ? `+506-${String(formData.telefono).trim()}`
    : `+${String(formData.telefonoPrefijo).trim()}-${String(formData.telefono).trim()}`;
        ...
        {tipoTelefono === 'extranjero' ? (
          <div>
            ...
            <div className="flex w-full">
              {/* Prefijo + símbolo + número todo alineado horizontalmente y del mismo tamaño que el nacional */}
              <div className="flex items-center px-3 border border-r-0 border-gray-300 bg-gray-100 text-gray-500 text-sm rounded-l-md">
                +
              </div>
              <input
                type="text"
                ...
                disabled={!editando}
                placeholder="Pref"
                className="w-16 px-3 py-2 border-t border-b border-gray-300 text-gray-900 sm:text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
              <input
                type="tel"
                name="telefono"
                ...
                disabled={!editando}
                placeholder="Número"
                className="flex-grow px-3 py-2 border border-gray-300 rounded-r-md text-gray-900 sm:text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
            {(errores.telefonoPrefijo || errores.telefono) && (
              <p className="mt-1 text-sm text-red-500">{errores.telefonoPrefijo || errores.telefono}</p>
            )}
          </div>
        ) : (
          <div>
