...
//                 <h3 className="text-lg font-semibold break-all">Detalle: {precario.Detalle || "Sin descripción"}</h3>
...
const PrecariosList = () => {
  ...
  // };
const handlePreviewPdf = (filePath: string) => {
  const fullUrl = `${ApiRoutes.urlBase}/uploads/${filePath}`;
  window.open(fullUrl, '_blank', 'width=800,height=600');
};
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2">
          {precarios.length === 0 ? (
            <div className="bg-white rounded-lg shadow p-6 text-center">
              <p className="text-gray-500">No se encontraron solicitudes registradas</p>
            </div>
          ) : (
            precarios.map((precario) => (
              <div key={precario.id} className="bg-white rounded-lg shadow p-6">
                <h3 className="text-lg font-semibold break-words">Detalle: {precario.Detalle || "Sin descripción"}</h3>
                <p className="text-sm text-gray-500">Fecha: {precario.Date}</p>

                <div className="text-sm text-gray-500 mt-2">
                  <p>Archivos Adjuntos:</p>
<div className="flex flex-col gap-2 mt-2">
  {Array.isArray(precario.ArchivoAdjunto) &&
    precario.ArchivoAdjunto.map((file, index) => (
      <button
        key={index}
          ...
          {file.nombre || `Archivo ${index + 1}`}
                ...
                <p className="flex items-center gap-1 mt-4">
                  ...
                  <span className={`inline-flex items-center ${
                    ...
                    {precario.status === 'Aprobada' && <CheckCircleIcon className="h-4 w-4 mr-1" />}
                    {precario.status === 'Denegada' && <XCircleIcon className="h-4 w-4 mr-1" />}
                    {precario.status === 'Pendiente' && (
                      <span className="inline-block w-4 h-4 border-2 border-yellow-500 rounded-full mr-1"></span>
                    )}
                    {precario.status}
