...
export default function UsuarioPrecario() {
  ...
  //   if (uploadedFiles.length === 0 || !fileDescription.trim()) {
  //     MySwal.fire({
  //       title: 'Error',
  //       text: uploadedFiles.length === 0
  //         ? 'No has subido ningún archivo.'
  //         : 'Debes ingresar una descripción de los archivos.',
  //       icon: 'warning',
...
const handleSend = async () => {
  ...
  if (palabrasLargas.length > 0) {
    await MySwal.fire({
      ...
      text: 'Evita usar palabras con más de 30 caracteres seguidos en la descripción.',
  ...
  const handleBack = () => navigate('/mis-precarios');

  return (
    <div className="container mx-auto px-4 py-12">
      <style>{`
        .btn-azul { background-color: #2563eb !important; color: white !important; border: none !important; border-radius: 6px !important; padding: 8px 20px !important; font-weight: bold !important; }
        .btn-rojo { background-color: #dc2626 !important; color: white !important; border: none !important; border-radius: 6px !important; padding: 8px 20px !important; font-weight: bold !important; }
        .botones-horizontales { display: flex !important; justify-content: center; gap: 10px; }
      `}</style>
      ...
      <li className="flex items-center mb-4">
        ...
        <a href="/DocsPdf/Formulario Solicitud de Uso Precario.pdf" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
      ...
      <div {...getRootProps()} className={`p-8 border-2 border-dashed rounded-lg text-center cursor-pointer transition-colors ${isDragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'}`}>
        <input {...getInputProps()} />
        <p className="text-lg text-gray-500">{isDragActive ? 'Suelta los archivos aquí...' : 'Arrastra y suelta archivos PDF aquí, o haz clic para seleccionar archivos'}</p>
      </div>
      ...
      <div className="mt-8">
        <label htmlFor="fileDescription" className="block text-lg font-medium text-gray-700 mb-2">Descripción de los Archivos</label>
        <textarea id="fileDescription" maxLength={250} value={fileDescription} onChange={(e) => setFileDescription(e.target.value)} rows={4} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" placeholder="Escribe una descripción de los archivos que estás subiendo"></textarea>
      </div>
