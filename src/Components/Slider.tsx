...
export default function ImageSlider() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setMounted(true), 100); // Espera breve para que el navegador pinte la imagen primero
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mb-16">
      {!mounted ? (
        // Primera imagen optimizada para LCP
        <div className="aspect-video overflow-hidden rounded-lg shadow-md">
          <img
            src={images[0]}
            alt="Slide 1"
        ...
        // Slider completo renderizado después del primer paint
        <Slider {...settings}>
          {images.map((image, index) => (
            <div key={index} className="outline-none">
              <div className="aspect-video overflow-hidden rounded-lg shadow-md">
